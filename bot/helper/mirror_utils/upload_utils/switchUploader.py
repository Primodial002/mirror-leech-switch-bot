...
class SwUploader:
    ...
    async def __user_settings(self):
        self.__as_doc = self.__listener.user_dict.get("as_doc", False) or (
            config_dict["AS_DOCUMENT"]
            if "as_doc" not in self.__listener.user_dict
            else False
        )
        self.__lprefix = self.__listener.user_dict.get("lprefix") or (
            config_dict["LEECH_FILENAME_PREFIX"]
            if "lprefix" not in self.__listener.user_dict
            else ""
        )
    ...
    async def upload(self, excluded_files, size):
        ...
        for dirpath, _, files in sorted(await sync_to_async(walk, self.__path)):
            ...
            for file_ in natsorted(files):
                ...
                try:
                    ...
                    if (
                        self.__listener.seed
                        and file_ in excluded_files
                        and f_size in list(excluded_files.values())
                    ):
                        continue
        ...
        await self.__listener.onUploadComplete(
            None, size, self.__corrupted, self.__total_files, _, self.name
        )
    ...
    async def __upload_file(self, description, file):
        ...
        self.__sent_msg = await self.__sent_msg.reply_media(
        ...
        self.__sent_msg = await editMessage(
            self.__sent_msg, f"<copy>{file}</copy>\nMime Type: {mime_type}", button
        )
